...
We systematically investigate Generative Adversarial Networks (GANs) for synthetic data augmentation, evaluating DCGAN and cDCGAN architectures across four loss functions (Hinge, Wasserstein, BCE, MSE) on the ISIC skin lesion dataset. 
...
We began with an exhaustive grid search systematically exploring generator and discriminator learning rates, considering all combinations\ref{tab:lr_configs}. This generated four distinct configurations, each trained for 25 epochs and evaluated by FID score and classifier metrics at convergence. 
