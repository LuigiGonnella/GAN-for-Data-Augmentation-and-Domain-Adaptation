...
### Key Challenge
Severe class imbalance coupled with limited malignant samples (only 1,400 training) creates fundamental data scarcity for minority class learning. Traditional augmentation (geometric transformations) provides limited benefit; semantic diversity via synthetic generation is essential.
...
- Stage 1: Grid search over learning rates (identified optimal g_lr = d_lr = 2×10⁻⁴)
- Stage 2: Random search over dropout rates {0.0, 0.2, 0.3} and normalization strategies
...
- Baseline dataset + synthetic malignant samples (generated by optimal cDCGAN)
...
Trained ResNet18 exclusively on synthetic cDCGAN malignant samples and evaluated on real test set:
- **Accuracy**: 79.10%
- **Recall**: 80.33%
- **Precision**: 33.71%

**Findings**: Synthetic-only training achieved competitive recall, validating that synthetic samples captured genuine malignant lesion features rather than mode-collapsed artifacts. The reduced precision is clinically acceptable for screening applications.
...
- Synthetic-only training (ResNet18 on synthetic cDCGAN) achieves 80.33% recall on real test samples
- Validates diagnostic utility of synthetic samples without problematic domain shift
- Precision-recall trade-off acceptable for clinical screening applications
...
5. **Diagnostic Relevance**: Synthetic samples captured meaningful malignant features as evidenced by synthetic-only training performance, not just statistical artifacts

## Challenges and Solutions

| Challenge | Solution | Result |
|-----------|----------|--------|
| Severe class imbalance (7.5:1) | Synthetic augmentation + weak baselines | +18-24% recall improvements |
| Limited malignant data (1,000 samples) | Hinge loss + Spectral Normalization | Stable training (FID 193.49) |
| GAN training instability | Two-stage hyperparameter optimization | Robust configuration across architectures |
| Loss function sensitivity | Systematic evaluation of four losses | Hinge + SN generalized without tuning |
| High false negative rate | Augmentation + threshold optimization | 63.67% → 80%+ recall achieved |
...
python evaluation/domain_shift_evaluation.py --synthetic_only_model <path> --real_test_data data/test/
```
